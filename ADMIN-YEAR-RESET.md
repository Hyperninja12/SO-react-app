...
- **SO slip format:** `YY-00001` (e.g. `27-00001`). Sequence resets each calendar year.
- **Close Year:** Admin-only action that:
  ...
  3. **Prepares** the next year's counter so new slips use e.g. 27-00001.
...
1. **New slip:** Opening "New Work Slip" (without a draft) fetches the next SO number from the API in format `YY-00001`.
